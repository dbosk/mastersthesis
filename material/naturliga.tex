...
\begin{axiom}
	...
	För alla naturliga tal \(n\) gäller att dess efterföljare \(S(n)\) är
	ett naturligt tal.
\end{axiom}
...
\begin{axiom}
	\label{ax:NaturligaAxiomOandligt}
...
\axref{ax:NaturligaAxiomOandligt} säger att inget naturligt tal kan ha
\(0\) som efterföljare.
...
(\(0\)) som enligt \axref{ax:NaturligaAxiomOandligt} inte är efterföljare till
något naturligt tal.
...
\begin{definition}
	För alla naturliga tal \(a\), låt \(S^n(a)\) beteckna
	\begin{equation*}
		S^n(a) = \underbrace{S(S(S(\cdots S}_n(a)
	\end{equation*}
	där \(n\) betecknar antalet \(S\).
	Låt speciellt \(S^0(a)=a\).
\end{definition}

Vi avslutar detta avsnitt med att införa några välbekanta beteckningar.
\begin{definition}
...
\index{aritmetik}
\noindent
\lettrine{O}{rdet aritmetik kommer} från grekiskans
% TODO fixa fulhack för grekiska arithmoz
\(\alpha\rho\iota\theta\mu o\zeta\), som betyder tal.
...
\begin{definition}[Multiplikation]
	...
	\end{align}
	Multiplikationen \(a\cdot b\) skrivs vanligen som \(ab\).
\end{definition}
\begin{exercise}
...
Istället för att skriva ut alla faktorer \(b\)-faktorer i en produkt \(b\cdot
...
\begin{definition}
	...
	Då skriver vi produkten \(a\cdot a\cdots a\) med \(n\) faktorer som
	\(a^n\).
	Alltså,
	\begin{equation*}
...
\noindent
\lettrine{D}{et sista begreppet} vi ska introducera i detta kapitel är
olikheter.
...
\begin{definition}[Olikhet]
	...
	Då säger vi att \(a\) \emph{är mindre än} \(b\) om det finns ett naturligt
	tal \(n>0\) större än noll sådant att \(a+n=b\), vi skriver detta som
	\(a<b\).
	Vi kan också säga att \(b\) \emph{är större än} \(a\) och beteckna detta
	genom \(b>a\).
	Om vi tillåter att \(n\) kan vara noll, då skriver vi \(a\leq b\)
	respektive \(b\geq a\).
	Vi utläser dessa som \(a\) \emph{är mindre än eller lika med} \(b\)
	respektive \(b\) \emph{är större än eller lika med} \(a\).
\end{definition}
