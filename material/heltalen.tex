...
Om vi adderar två tal \(a\) och \(b\) får vi ett trejde tal \(c\), vi skriver
detta som \(a+b=c\).
...
\begin{definition}
	\label{def:invers}
	\index{invers}
	Givet en mängd med en definierad binär operation \(\diamond\) och ett
	identitetselement \(e\).
	Ett element \(b\) kallas för \emph{inversen till \(a\)} om
...
% UTÖKNINGEN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Utökningen av de naturliga talen\maybeskip}
...
Låt oss nu införa en ekvivalensrelation \(\sim\) på denna mängd.
Om \((a,b)\) och \((c,d)\) är ordnade par av naturliga tal, det vill säga
element i mängden \(M\), då vi vill att \((a,b)\sim (c,d)\) ska vara sant
precis när \(a+d=b+c\).
\begin{exercise}
	Kan vi vara säkra på att ekvivalensrelationen \(\sim\) faktiskt är en
	ekvivalensrelation?
\end{exercise}
...
\end{example}
\begin{remark}
	Kom ihåg att det spelar ingen roll vilket talpar från en ekvivalensklass
	som vi väljer som representant.
\end{remark}
...
\begin{remark}
	Mängden av ekvivalensklasser under ekvivalensrelationen \(\sim\) på mängden
	\(M\) är inte densamma som mängden \(M\) och brukar normalt betecknas med
	\(M/\sim\).
	Mängden \(M=\{(a,b) : a \text{ och } b \text{ är naturliga tal}\}\), det
	vill säga \(M\) innehåller ordnade par av naturliga tal, medan mängden
	av alla ekvivalensklasser är \(\{[(a,b)] : (a,b)\in M\}\) och alltså
	innehåller mängder.
\end{remark}
...
Vi har nu tal med egenskapen att om vi adderar dem får vi identitetselementet.
...
\begin{definition}[Heltalen]
	...
	vara en ekvivalensrelation på mängden \(M\) sådan att \((a,b)\sim (c,d)\)
	är uppfylld för element \((a,b)\) och \((c,d)\) i \(M\) om \(a+d=b+c\).

	Låt \(\Z=\{[(a,b)] : (a,b)\in M\}\) vara mängden av alla ekvivalensklasser
	för \(M\) under ekvivalensrelationen \(\sim\).
...
Den definition som är den naturliga efterföljaren till olikheter är denna.
\begin{definition}
...
\end{definition}
\begin{remark}
	Låt \(a\) och \(b\) vara heltal och \(-b\) är den additiva inversen till
	\(b\).
	...
	från \(a\).
	Ur en matematisk synvinkel adderar vi den additiva inversen för \(b\) till
	\(a\).
\end{remark}
...
\begin{proof}
	Vi har att \(1+(-1) = 0\).
	Om vi multiplicerar noll med ett tal får vi fortfarande noll.
...
\begin{remark}
	...
	i detta bevis använder oss av de algebraiska egenskaperna givna i
