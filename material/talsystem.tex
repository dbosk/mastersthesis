...
De tecken vi är vana vid i västkulturerna är siffrorna \(0\) till \(9\).
Vi behöver här skilja på ett tal och en siffra.
Siffror är tecken som används för att representera tal.
Det vill säga, talet \(123\) består av de tre siffrorna \(1,2\) och \(3\).
...
Som kan ses i \tblref{tbl:RomerskaTal2} är det eftersträvansvärt att så få
...
\begin{proof}
	...
	\end{equation}
	Eftersom att vänsterledet i \eqref{eq:NollEndastOmLika} är delbart med
	...
	Alltså är \(-(c_n-d_n)\) delbar med \(b\) endast om \(-(c_n-d_n)=0\) vilket
	är en motsägelse.
...
\begin{proof}[Bevis \thmref{thm:PositionssystemAllaUnika}]
	...
	Från \xrcref{xrc:KvotenMindre} har vi att \(q_{i+1}<q_i\) eftersom att
	\(b>1\).
	...
	en motsägelse.
	Alltså måste det vara samma representation.
...
\begin{example}
	...
	Representationen för tal i det romerska systemet är alltså inte unik.
\end{example}
...
resultat.
Till exempel när vi i \exref{ex:AdderaMedRest} steg \verb'b)' adderar \(3\)
...
\thmref{thm:PositionssystemAllaUnika} att summan är ensiffrig.
