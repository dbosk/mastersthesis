...
Vi fortsätter med en annan definition med stor koppling till
delmängdsbegreppet.
\begin{definition}
...
\begin{example}
	...
	till \(M\).
	Vi har dessutom att \(A^\complement = M\setminus A = \{3\}\).
\end{example}
...
potensmängd och definieras härnäst.
\begin{definition}
...
Enligt denna definition skulle alltså likhetsrelationen mellan mängder vara en
...
delmängder kallade partitioner, dessa partitioner utgörs av något som brukar
...
\begin{definition}
	...
	denna som \([a]_R\), \(a\) sägs vara \emph{representanten} för
	ekvivalensklassen.
\end{definition}
...
\begin{example}
	...
	Alltså är relationen transitiv.
	Relationen uppfyller alltså kraven för en ekvivalensrelation och måste
	...
	samma art.
	Exempelvis mängden av alla berguvar.
\end{example}
...
\begin{definition}
	...
	Låt \(A\) och \(B\) vara mängder.
	En \emph{funktion}, eller \emph{avbildning}, \(f:A\to B\) tilldelar till
	varje \(a\in A\) ett välbestämt \(b\in B\).
	Vi skriver \(f(a)=b\) eller \(a\mapsto b\) och säger att \(a\)
	\emph{avbildas} på \(b\) eller att \(b\) är \emph{bilden} av \(a\).
\end{definition}
\begin{remark}
	Notera att vi kan se funktionen \(f\) som en delmängd till den kartesiska
	produkten \(A\times B\).
	Det vill säga \(f=\{(a,b)\in A\times B : a\mapsto b\}\).
	Då är \((a,b)\in f\) om \(a\mapsto b\), eller \(f(a)=b\).
\end{remark}
\begin{example}
...
I de flesta fall är det inte lämpligt att tala om hur varje element avbildas
utan istället ge en beskrivning av hur elementen ska avbildas.
\begin{example}
	Låt \(M\) vara mängden av alla människor i Sverige och \(P\) vara mängden
	av alla geografiska platser i Sverige.
	Låt \(p:M\to P\) vara en avbildning från \(M\) till \(P\).
	Vi avbildar då varje människa i \(M\) på den geografiska plats i \(P\) där
	den för tillfället befinner sig.
\end{example}
...
\begin{example}
	Mängderna \(M=\{a,b,c\}\) och \(N=\{1,2,3\}\) har samma kardinalitet, det
	vill säga \(\card M = \card N\), eftersom att avbildningen
	\(\{(a,1),(b,2),(c,3)\}\) är bijektiv.
\end{example}
